...
def compute_gae(
    ...
    values  = transitions.value    # (T, n_envs)
    dones   = transitions.done.astype(jnp.float32)  # (T, n_envs)
    T = rewards.shape[0]
...
class IPPOTrainer:
    ...
    def _make_train_state(
        ...
        def _init(k, init_params=None, init_opt_state=None):
            ...
            ts = TrainState.create(apply_fn=self.network.apply, params=params,
                                   tx=optimizer)
    ...
    def _collect_rollout(
        ...
        def _scan_step(carry, _):
            ...
            if self.reward_mode == "delivery":
                ...
                delivery = rewards["agent_0"] / float(DELIVERY_REWARD)  # +1 per delivery
                r0 = r1 = delivery
            ...
            tr0 = Transition(obs=obs0, action=a0, log_prob=lp0,
                             value=val0, reward=r0, done=done, hidden=h0)
            tr1 = Transition(obs=obs1, action=a1, log_prob=lp1,
                             value=val1, reward=r1, done=done, hidden=h1)
            ...
            new_carry = (new_obs, new_states, new_h0, new_h1, key,
                         ep_ret * (1 - done), ep_len * (1 - done))
            return new_carry, (tr0, tr1, ep_ret * done, ep_len * done, done, comp0)

        init_carry = (obs_dict, env_states, h0, h1, key,
                      jnp.zeros(self.n_envs), jnp.zeros(self.n_envs, dtype=jnp.int32))

        final_carry, (trs0, trs1, ep_returns, ep_lens, dones, shaped_comps) = jax.lax.scan(
            _scan_step, init_carry, None, length=self.rollout_len
        )
        ...
        return (trs0, trs1,
                ...
                (final_h0, final_h1),
                ep_returns, ep_lens, dones, shaped_comps, key)
    ...
    def _make_mb_update(self):
        ...
        @jax.jit
        def _mb_update(ts, obs, hidds, acts, lps, old_vals, advs, rets,
                       clip_eps, vf_coef, ent_coef):
            loss_fn = lambda p: ppo_loss(
                p, ts.apply_fn, obs, hidds, acts, lps, old_vals, advs, rets,
                clip_eps, vf_coef, ent_coef,
            )
        ...
        for _ in range(self.n_epochs):
            ...
            for start in range(0, B, self.batch_size):
                ...
                ts, loss = self._mb_update(
                    ts, obs[b], hidds[b], acts[b], lps[b], old_vals[b], advs[b], rets[b],
    ...
    def train(self, key: jnp.ndarray, resume_params: dict = None,
              log_callback=None, checkpoint_dir=None, checkpoint_every: int = 100):
        ...
        while total_collected < self.total_steps:
            ...
            (trs0, trs1,
             (obs_dict, env_states),
             (h0, h1),
             ep_returns, ep_lens, ep_dones, shaped_comps, key) = self._collect_rollout(
                ts0, ts1,
                (obs_dict, env_states),
                (h0, h1),
                k_rollout,
            ...
            finished = ret_np[done_np]               # returns for completed episodes
            all_ep_returns.extend(finished.tolist())
            # Also track mean per-step reward for updates with no episode completions
            step_r_np = np.asarray(trs0.reward)      # (T, n_envs) — agent_0 reward (delivery=20)
