...
from .ippo import IPPOTrainer, calculate_gae
...
from ..level_generator import EnvParams, ParametrizedOvercooked


# ── Visualization helpers ─────────────────────────────────────────────────────

def _save_layout_snapshot(grid: np.ndarray, step: int, out_dir: Path) -> None:
    from ..level_generator import render_env as _render_env
...
def _save_agent_gif(grid: np.ndarray, network, params_0, params_1,
    ...
    import imageio
    from ..environment import Layout, OvercookedV2, StaticObject
    ...
    try:
        ...
        print(f"[SFL] GIF skipped (visualizer not available): step={step:,}", flush=True)
...
class SFLTrainer:
    ...
    def get_learnability_set(self, ts0, ts1, key):
        print(f"[SFL] Creating learnability set. Generating {self.n_random_pool} random levels for evaluation...")
        key, k_gen = jax.random.split(key)
        gen_keys = jax.random.split(k_gen, self.n_random_pool)
        ...
        for i in range(self.n_random_pool):
            ...
            level = OvercookedLevel(
                grid=grids_np[i], params=params, seed=i, score=scores[i],
            )
            scored_levels.append(level)

        scored_levels.sort(key=lambda lv: lv.score, reverse=True)

        new_buffer = LevelBuffer(capacity=self.buffer.capacity)
        for lv in scored_levels[:self.buffer.capacity]:
    ...
    def train(
        ...
        _log(f"[SFL] Training started (target: {total_target:,} steps)\n")

        while total_collected < total_target:
            self.iterations += 1
            _log(f"\n[SFL] ── iteration {self.iterations} " + "─" * 40)

            # Step 1: Update buffer
            _log(f"[SFL] [1/5] refreshing level buffer ({self.n_random_pool} candidates)...")
            t_eval = time.time()
            self.buffer, key = self.get_learnability_set(ts0, ts1, key)
            _log(f"[SFL] [1/5] buffer updated: {self.buffer.size} levels in {time.time()-t_eval:.1f}s")
            ...
            for t in range(T_steps):
                ...
                _log(
                    f"[SFL] [2/5] upd {self.updates + 1} (t={t + 1}/{T_steps}) | "
                ...
                _log(f"[SFL] [3/5] collecting rollout ({self.ippo.rollout_len} steps x {N_L} envs)...")
                t_roll = time.time()
                ...
                _log(f"[SFL] [3/5] rollout done in {time.time()-t_roll:.1f}s")

                _log("[SFL] [4/5] computing GAE + PPO update...")
                t_upd = time.time()
                ...
                _log(f"[SFL] [4/5] update done in {time.time()-t_upd:.1f}s | loss={log_loss:.4f}")
                ...
                summary_line = (
                    f"[SFL] [5/5] iter={self.iterations:4d} | upd={self.updates:5d} | "
        ...
        _log(f"\n[SFL] Training complete. Steps: {total_collected:,}")
        if workflow_log_file:
