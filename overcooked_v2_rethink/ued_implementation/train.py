...
from .environment import OvercookedV2, overcooked_v2_layouts, Layout, StaticObject
from .training import SFLTrainer, ACCELTrainer
from .ued import OvercookedUEDEnv
from .level_generator import ParametrizedOvercooked
...
def plot_curves(log, out_dir, title, filename="training_curve.png",
    ...
    if is_curriculum:
        ...
        ax.set_ylabel("SFL Learnability"); ax.legend(fontsize=8); ax.grid(alpha=0.3)
...
def main():
    ...
    ap.add_argument("--sfl-pool-size", type=int, default=50)
    ...
    is_curriculum = args.sfl or args.accel
    ...
    # ── Environment setup ─────────────────────────────────────────────────
    if is_curriculum:
        ued = OvercookedUEDEnv(H=_h, W=_w, max_steps=400, agent_view_size=args.view_size)
        env = ued.env
