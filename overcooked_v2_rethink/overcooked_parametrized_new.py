...
def compute_min_cycle(grid: np.ndarray) -> int:
    ...
    agents      = [(r, c) for r in range(H) for c in range(W) if grid[r, c] in (AGENT_0, AGENT_1)]
    ingredients = [(r, c) for r in range(H) for c in range(W) if grid[r, c] in (INGREDIENT_0, INGREDIENT_1)]
    pots        = [(r, c) for r in range(H) for c in range(W) if grid[r, c] == POT]
    plates      = [(r, c) for r in range(H) for c in range(W) if grid[r, c] == PLATE_PILE]
...
class ParametrizedOvercooked:
    ...
    def _generate_from_key(self, candidate_key) -> Tuple[np.ndarray, "EnvParams"]:
        ...
        obs_density = float(jax.random.uniform(k_obs, minval=0.0, maxval=MAX_OBS_FRAC))
        obs_skew_x  = float(jax.random.uniform(k_res, minval=-1.0, maxval=1.0))
        res_density = float(jax.random.uniform(k_ng,  minval=0.0, maxval=MAX_RES_FRAC))
    ...
    def _generate_from_key_jax(self, candidate_key) -> Tuple[jnp.ndarray, jnp.ndarray]:
        ...
        obs_density = jax.random.uniform(k_obs, minval=0.0, maxval=float(MAX_OBS_FRAC))
...
# uv run python overcooked_parametrized_new.py --dims 5x9
