...
class ACCELTrainer:
    ...
    def train(
        ...
        while total_collected < total_target:
            self.iterations += 1
            d = (self.rng.random() < self.replay_prob) and (len(self.buffer) > 0) # decision whether to replay a level from the buffer, or generate a brand-new one
            _log(f"[ACCEL] iter {self.iterations}: decision = {'replay' if d else 'new level'}")

            if not d:
                # ── Generate new level, score it, add to buffer ───────────
                _log(f"[ACCEL] iter {self.iterations}: generating new level...")
                level, key = self._generate_level(key)
                if level is None:
                    continue
                pvl, key = self._score_level(ts0, ts1, level.grid, key)
                ...
                log_loss = 0.0
                log_level_score = pvl
                iter_deliveries = 0  # scoring-only rollout; deliveries not tracked for this branch
                ...
                _log(f"[ACCEL] iter {self.iterations}: replaying level from buffer...")
                ...
                iter_deliveries = int(np.sum(deliv_per_step))  # deliveries in this iteration's rollout
            ...
            # ── Logging — every iteration, so the console trace and the live
            # plot both stay populated from the first iteration onward rather
            # than only every log_every (or log_every*5) iterations.
            ...
            summary_line = (
                ...
                f"deliv_this_iter={iter_deliveries:4d}; deliv_total={sum(all_ep_deliveries):6d};  "
                f"loss={log_loss:.4f};  buf={len(self.buffer)}/{self.buffer.capacity};  "
                f"score={self.buffer.mean_score():.4f};  branch={'replay' if d else 'new':6s};  "
                f"replays={self.replay_count};  edits={self.edit_count}  "
                f"sps={sps:.0f}"
            )
            print(summary_line, flush=True)  # only console line printed every iteration
            _log(summary_line)
