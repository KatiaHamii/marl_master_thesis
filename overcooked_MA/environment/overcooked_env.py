...
class OvercookedV2(MultiAgentEnv):
    ...
    def reset(self, key: chex.PRNGKey, static_objects=None, agent_positions_xy=None) -> Tuple[Dict[str, chex.Array], State]:
        static_objs = self.layout.static_objects if static_objects is None else static_objects
        grid = jnp.stack(
            [
                static_objs,
                jnp.zeros_like(static_objs),
                jnp.zeros_like(static_objs),
            ],
            axis=-1,
            dtype=jnp.int32,
        ...
        state = State(
            ...
            terminal=False,
            recipe=recipe,
            new_correct_delivery=False, # at the very start of an episode, no delivery has happened yet.
        )

        key, key_rnd = jax.random.split(key)
        if self.random_reset:
            state = self._randomize_state(state, key_rnd) # full domain randomization.
        elif self.random_agent_positions:
            state = self._randomize_agent_positions(state, key_rnd) # randomizes only where the agents spawn

        return lax.stop_gradient(self.get_obs(state)), lax.stop_gradient(state) # no backpropagation through the environment
    ...
    def step(
        ...
        """Step the env and auto-reset on episode end, as rollout collectors expect.

        Without this, a rollout longer than one episode (rollout_len > max_steps)
        would leave the env frozen in its terminal state after the first episode
        ends, causing every remaining step to be mis-logged as a new (empty)
        episode. Curriculum layouts (SFL/ACCEL/EvoSFL) reset envs with a
        per-episode static_objects grid rather than self.layout, so the
        reset-on-done must reuse that same layout (state.grid[:, :, 0], the
        static channel, which step_agents never mutates) instead of falling
        back to self.layout.
        """
        key, key_reset = jax.random.split(key)
        obs_st, state_st, reward, done, info = self.step_env(key, state, actions)
        obs_re, state_re = self.reset(key_reset, static_objects=state.grid[:, :, 0])

        is_done = done["__all__"]
        state = jax.tree_util.tree_map(lambda re, st: jnp.where(is_done, re, st), state_re, state_st)
        obs = jax.tree_util.tree_map(lambda re, st: jnp.where(is_done, re, st), obs_re, obs_st)
    ...
    def get_obs(self, state: State) -> Dict[str, chex.Array]:
        ...
        def _agent_obs(agent_id):
            ...
            self_inv_ch  = _place(s_inv, sy, sx)           # ch 1 -> what agent is holding (0=empty, 1=plate, 2/3=ingredients, 6/7=dish)
            other_dir_ch = sum_dirs - self_dir_ch          # ch 2 -> teammate's direction
            other_inv_ch = sum_invs - self_inv_ch          # ch 3 -> teammate's inventory (what teammate is holding)
